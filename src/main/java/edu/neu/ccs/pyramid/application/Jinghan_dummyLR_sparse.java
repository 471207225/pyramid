 ...
 */
public class Jinghan_dummyLR_sparse {
                ...
                if(i%30==0){
                    try{
                        ...
                        for (int ind_checkWord = 0; ind_checkWord<checkNum; ind_checkWord++) {
                            ...
                            writer.println("");
                            writer.println(" word: \""+ wordNames.get(wordInd_write).getName()+ " \" \t weight "+ train_weights[wordInd_write] +" \t word score"+ optimizer.wordVectorRegression.scores(testWord2vec.getRow(wordInd_write)).toString()+" \t target gradient: " + gradients[wordInd_write]*shrinkage_tuned + " \t shrunked predict gradient: " +
